...
**v3 具体做法**：
```bash
# 先看真相（state.db）—— 推荐用 Python（更普及）
python .zeus/v3/scripts/run.py --status

# 如果 Python 不可用，可用 Node.js 替代
...
| "看看现在进度" / "Status?" / "到哪了" | **status** | 运行 `python .zeus/v3/scripts/run.py --status`（v3）或 `python .zeus/scripts/zeus_runner.py --status` → 报告进度 |
...
**status**
1. Run `python .zeus/v3/scripts/run.py --status` (v3, recommended) or `python .zeus/scripts/zeus_runner.py --status`
...
|---|---|
| `python .zeus/v3/scripts/run.py --status` | **v3 status intent (recommended)** |
| `python .zeus/v3/scripts/run.py --plan` | v3 plan preview |

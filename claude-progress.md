...
- **已完成：整理 Phase/Milestone 结构**
  - 问题：M-V3-003 一个 milestone 塞了 14 个 task，横跨 5 个完全不同的功能域
  - 废弃的 `roadmap.json` 与数据库不同步，已删除
  - **执行结果**：
    1. 备份 `state.db` ✅
    2. 数据库重组：删除旧 3 Phase/3 Milestone，新建 4 Phase + 8 Milestone
       ...
       - P-V3-004 Quality & Standards → M-V3-007 Cleanup & Docs (T-027, T-028), M-V3-008 Engineering Standards (T-029, T-032)
    3. 更新所有 17 个 task 的 `milestone_id`
    4. Dashboard 钻取验证通过：Phase → Milestone → Task 三级关联正确
    5. `--export-plan` 生成新 `task.json`（4 phases / 8 milestones / 17 tasks）
    6. 删除废弃 `roadmap.json`（`.zeus/main/roadmap.json` + `.zeus/v3/roadmap.json`）
    7. 测试通过：`test_phase_crud`, `test_milestone_crud`, `test_import_phases_and_milestones`
  - **Commit**: `70253ea`
